...
const OrderCard = ({ order, colId, tracking, hasHardcopy, onAction, onView }: OrderCardProps) => {
  ...
  const getMenuItems = (): { label: string; action: ActionType; color: string; icon: JSX.Element }[] => {
    switch (colId) {
      ...
      case 'onsite': {
        const primary = isStandby
          ? { label: 'Start Job', action: 'start' as ActionType, color: 'text-emerald-600 dark:text-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-500/10',
          ...
          : { label: 'Complete Job', action: 'complete' as ActionType, color: 'text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-500/10',
        ...
        {order.items && order.items.length > 0 && (
          <div className="border-t border-slate-100 dark:border-white/5 pt-2.5">
            <div className="flex items-center gap-1.5 mb-2">
              ...
              <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Tools</span>
            </div>
            <div className="space-y-1.5">
              {order.items.filter((i: any) => i.tool).map((item: any) => (
                <div key={item.id} className="flex flex-col gap-1 p-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-white/5">
                  <span className="font-black text-[13px] text-slate-900 dark:text-white truncate leading-tight" title={item.tool.name}>
                    {item.tool.name}
                  </span>
                  <span className="text-[10px] text-slate-500 font-mono tracking-wider">
                    {item.tool.serialNumber}
                  </span>
                </div>
              ))}
            </div>
