              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {form.status === 'onsite' && (
                      <p className="mt-1.5 text-[10px] text-amber-600 dark:text-amber-400 font-bold flex items-center gap-1">
                        ...
                        Onsite status should be set via Orders. Manually setting Onsite requires a valid active order.
                      </p>
                    )}
                    {form.status === 'available' && editingId && (() => {
                      const orig = tools.find(t => t.id === editingId);
                      return orig?.status === 'onsite';
                    })() && (
                      <p className="mt-1.5 text-[10px] text-rose-600 dark:text-rose-400 font-bold flex items-center gap-1">
                        <svg className="w-3 h-3 shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" /></svg>
                        Moving from Onsite to Yard should be done via the Orders "Return Tools" flow to log proper documentation.
                      </p>
                    )}
                  </div>
                </div>
